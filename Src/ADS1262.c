...
void ads_init_default(ads1262_t *dev, SPI_HandleTypeDef *hspi,
  ...
  dev->cs_port = cs_port;
  dev->cs_pin = cs_pin;
  dev->drdy_port = drdy_port;
  dev->drdy_pin = drdy_pin;
  dev->start_port = start_port;
  dev->start_pin = start_pin;
  dev->pwdn_port = pwdn_port;
  ...
  ads_delay(1);
  if (ads_reg_write_and_check(dev, MODE2, (uint8_t)0x85) !=
      0) { /* DR = 50 SPS, bypass PGA */
...
void ads_self_calibration(ads1262_t *dev) {
  ...
  uint32_t offset_code = ((uint32_t)offsetcal0 << 16) |
                         ((uint32_t)offsetcal1 << 8) | (uint32_t)offsetcal2;
  ads_port_print_string("OFFSETCAL Code: 0x%06X\n", offset_code);
}

void ads_select_input(ads1262_t *dev, uint8_t muxp, uint8_t muxn) {
  ads_reg_write_and_check(dev, INPMUX,
                          (uint8_t)(((muxp << 4) & 0xF0) | (muxn & 0x0F)));
}
